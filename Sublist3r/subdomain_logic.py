...
import validators
import logging
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QLabel, QLineEdit, QPushButton, QTextEdit, QFileDialog, QProgressBar, QHBoxLayout, QSpinBox, QStatusBar, QMessageBox)
...
class SubdomainEnumerator(QMainWindow):
    ...
    def initUI(self):
        ...
        self.domainInput.setToolTip('Enter the domain to enumerate subdomains.')
        ...
        self.enumerateButton.setToolTip('Click to start enumerating subdomains.')
        ...
        self.saveButton.setToolTip('Click to save the enumerated subdomains to a file.')
        ...
        self.itemsPerPageSpinBox.valueChanged.connect(self.update_items_per_page)
    ...
    def enumerate_subdomains(self):
        ...
        if not os.path.isfile(self.script_path):
            self.resultArea.append(f'Script file does not exist: {self.script_path}')
            return

        if domain:
            domain = self.normalize_domain(domain)
            if not self.validate_domain(domain):
                self.resultArea.append('Invalid domain format. Please enter a valid domain.')
                return
            ...
            self.enumerationThread = SubdomainEnumerationThread(domain, self.script_path)
            self.enumerationThread.finished.connect(self.on_enumeration_finished)
            self.enumerationThread.error.connect(self.on_enumeration_error)
            self.enumerationThread.progress.connect(self.on_progress)
            self.enumerationThread.update_progress.connect(self.on_update_progress)
    ...
    def update_pagination(self):
        total_pages = (len(self.subdomains) + self.items_per_page - 1) // self.items_per_page
        self.pageLabel.setText(f'Page {self.current_page + 1}/{total_pages}')
        self.prevButton.setEnabled(self.current_page > 0)
        self.nextButton.setEnabled((self.current_page + 1) * self.items_per_page < len(self.subdomains))
    ...
    def update_items_per_page(self):
        ...
        self.settings.setValue("itemsPerPage", self.items_per_page)
        self.show_page(self.current_page)
    ...
    def save_results(self):
        ...
        file_name, _ = QFileDialog.getSaveFileName(self, "Save Subdomains", "", "Text Files (*.txt);;All Files (*)", options=options)
        if file_name:
            try:
    ...
    def set_script_path(self):
        options = QFileDialog.Options()
        script_path, _ = QFileDialog.getOpenFileName(self, "Select Sublist3r Script", "", "Python Files (*.py);;All Files (*)", options=options)
        if script_path:
            self.script_path = script_path
            self.settings.setValue("scriptPath", self.script_path)
            self.statusBar.showMessage(f'Script path set to: {self.script_path}')
            logging.info(f"Script path set to: {self.script_path}')
            # Verify the script path immediately
            if not os.path.isfile(self.script_path):
                logging.error(f"Script file does not exist: {self.script_path}")
            else:
                logging.info(f"Verified script file exists: {self.script_path}")
